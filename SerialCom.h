...
void sendSerialTelemetry() {
 switch (queryType) {
  ...
  case 'n': // battery monitor
          readSerialCommand();
//        SERIAL_PRINT("waypoint no. from 0 onwards");
//        missionNbPoint = readIntegerSerial();
//        SERIAL_PRINT("Latitude value");
//        waypoint[missionNbPoint].latitude = readIntegerSerial();
//        SERIAL_PRINT("Longitude value");
//        waypoint[missionNbPoint].longitude = readIntegerSerial();
//        SERIAL_PRINT("Altitude value");
//        waypoint[missionNbPoint].altitude = readIntegerSerial();        
#ifdef BattMonitor
  ...
  case 'q': // Send Vehicle State Value
    ...
    //PrintValueComma(baroRawAltitude - baroGroundAltitude);
   // PrintValueComma(rangeFinderRange[ALTITUDE_RANGE_FINDER_INDEX]);
    //PrintValueComma(countSudeep = millis());
    SERIAL_PRINTLN();
    break;

  case 'r': // Vehicle attitude                        // Sudeep added 'r' for GPS log data
    //PrintValueComma(kinematicsAngle[XAXIS]);
    //PrintValueComma(kinematicsAngle[YAXIS]);
    //SERIAL_PRINTLN(getHeading());
   //Debug for heading hold:-
    PrintValueComma((float)adjustedGyroHeading);
    PrintValueComma((float)getAbsoluteHeading()); 
    PrintValueComma((float)filtered_heading);
    PrintValueComma(headingHoldState);
    PrintValueComma((float)heading);
    PrintValueComma((float)trueNorthHeading);
    PrintValueComma(receiverCommand[AUX2]);
//    PrintValueComma((float)measuredMagX);
 //   PrintValueComma((float)measuredMagY);
  //  PrintValueComma((float)measuredMagZ);
   /* 
   //end bebug for heading hold
   //Debug for altitude hold:-
   //PrintValueComma((float)sonarAltitudeToHoldTarget);
   //PrintValueComma((float)getBaroAltitude());
   ...
   PrintValueComma(receiverCommand[3]);
   PrintValueComma(throttle);
   PrintValueComma(kinematicsAngle[XAXIS]);
   PrintValueComma(kinematicsAngle[YAXIS]);
    
   //PrintValueComma((float)altitudeHoldState);
   ...
   for (byte motor = 0; motor < LASTMOTOR; motor++) {
    ...
    /*
    PrintValueComma(navigationState);
    PrintValueComma(positionHoldState);
    PrintValueComma(waypointIndex);
    PrintValueComma(currentPosition.latitude);
    PrintValueComma(homePosition.latitude);
    PrintValueComma(missionPositionToReach.latitude);
    PrintValueComma(distanceToDestination);
    PrintValueComma(angleToWaypoint);
    PrintValueComma(gpsYawAxisCorrection);
    PrintValueComma(gpsPitchAxisCorrection);
    PrintValueComma(gpsRollAxisCorrection);
    PrintValueComma(trueNorthHeading);
    PrintValueComma(sin(angleToWaypoint));
    PrintValueComma(cos(angleToWaypoint));
    //for (byte motor = 0; motor < LASTMOTOR; motor++) {
      //PrintValueComma(motorCommand[motor]);
    //}
    SERIAL_PRINTLN();
  ...
  case 's': // Send all flight data
    ...
    PrintValueComma(getReceiverSIData(YAXIS));
    PrintValueComma(-gyroRate[YAXIS]);
    PrintValueComma(getReceiverSIData(YAXIS)+gyroRate[YAXIS]);
    SERIAL_PRINTLN();
...
//     for (byte axis = 0; axis < LASTCHANNEL; axis++) {
 ...
 //   PrintValueComma(navigationState);
    ...
    // Sudeep GPS navigation status
    PrintValueComma(haveAGpsLock());
    ...
    PrintValueComma(waypointIndex);
    PrintValueComma(distanceToDestination);
    PrintValueComma(angleToWaypoint*57.3);
    PrintValueComma(missionPositionToReach.latitude);
    PrintValueComma(missionPositionToReach.longitude);
    PrintValueComma(missionPositionToReach.altitude);
    PrintValueComma('#');
    PrintValueComma(homePosition.latitude);
    PrintValueComma(homePosition.longitude);
    PrintValueComma(homePosition.altitude);
    PrintValueComma('#');
    PrintValueComma(getBaroAltitude());
    PrintValueComma(altitudeHoldState);    
    PrintValueComma(baroAltitudeToHoldTarget);
    //PrintValueComma(haveNewGpsPosition());
    PrintValueComma(positionHoldState);    
    PrintValueComma(navigationState);
    PrintValueComma('#');
    //PrintValueComma(receiverCommand[AUX1]);    
    //PrintValueComma(countSudeep);
    PrintValueComma(positionHoldPointToReach.latitude);
    PrintValueComma(positionHoldPointToReach.longitude);
    PrintValueComma(positionHoldPointToReach.altitude);
    SERIAL_PRINTLN();
    break;
  ...
  case 'w': // Report LeaderFollower
     
     #ifdef UseGPSNavigator
       #ifdef LeaderFollower
         #ifdef Follower    
          
          readWaypointLF.latitude = readIntegerSerial();
          readWaypointLF.longitude = readIntegerSerial();
        //readWaypointLF.altitude = readIntegerSerial();
          
          //if(191332079 < readWaypointLF.latitude < 191355806)  //Geodatic fence to main ground in IITB
            waypointLF.latitude=readWaypointLF.latitude;
          //if(729108305 < readWaypointLF.longitude< 729140223) //Geodatic fence to main ground in IITB
