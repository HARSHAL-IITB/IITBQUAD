...
void sendSerialTelemetry() {
          ...
          SERIAL_PRINT('w');
           SERIAL_PRINT(1);
           SERIAL_PRINT(';');
          SERIAL_PRINT(191627012);//gpsData.lat);           
          SERIAL_PRINT(';');
          //SERIAL_PRINT('l');
          SERIAL_PRINT(723252452);//gpsData.lon);
          SERIAL_PRINT(';');
          //PrintValueComma(2);
         // SERIAL_PRINTLN();
     #endif
...
//    #endif
  ...
  case 'q': // Send Vehicle State Value
    ...
    //queryType = 'X';
      ...
      for (byte channel = 0; channel < 4; channel++) { // Configurator expects 8 values
  ...
  case 'r': // Vehicle attitude                        // Sudeep added 'r' for GPS log data
   ...
   PrintValueComma((float)getHeading()); 
    SERIAL_PRINTLN(filtered_heading,5);
    /*
  ...
  case 's': // Send all flight data
    ...
    PrintValueComma(receiverCommand[MODE]);
    ...
    PrintValueComma(altitudeHoldCorrectionSudeep);
    PrintValueComma(getBaroAltitude() - baroAltitudeToHoldTarget);
    ...
    //PrintValueComma(homePosition.latitude);
    //PrintValueComma(missionPositionToReach.latitude);
    //PrintValueComma(previousaltitudeHoldState);    
    //PrintValueComma(countToInitHome);
    //PrintValueComma(baroGroundAltitude);
    //PrintValueComma(haveAGpsLock());
    //PrintValueComma(PID[GPSROLL_PID_IDX].P);
    //PrintValueComma(PID[GPSYAW_PID_IDX].P);
    //PrintValueComma(navigationState);
    //PrintValueComma(positionHoldState);
    SERIAL_PRINTLN();
...
//    
  ...
  case 'w': // Report LeaderFollower
       ...
       #ifdef LeaderFollower
         #ifdef Follower    
                            lon = readIntegerSerial();
          waypointLF.latitude = readIntegerSerial();
          //if (SERIAL_AVAILABLE()) {
           //  lon = SERIAL_READ();}
          
         // if(lon=='l')  
          {waypointLF.longitude = readIntegerSerial();}
         // else
         //  waypointLF.longitude = 3;
          
          //waypointLF.altitude = readIntegerSerial();
        // Serial.print("Lat: ");
         Serial.print(waypointLF.latitude);
         Serial.print(',');
         Serial.println(waypointLF.longitude);
         //Serial.print(',');
         //Serial.println(waypointLF.altitude);
